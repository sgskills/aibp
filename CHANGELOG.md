...
- 电商能力分轨由 `commerce` 更名为 `ecommerce`，明确覆盖中国市场与跨境市场的电商经营、投放及后续专业能力。
- `core`、`ecommerce`、`tooling` 仍然只是理解、合同与路由标签；所有 Skill 继续平铺在 `skills/<slug>`，不新增物理分类目录。
- README 中英文版、Skill 目录和仓库校验合同已同步新分轨，并增加旧分轨残留检查。

### 拟人化命名

- `sg-skill-optimizer` 的正式中文展示名由“SG Skill 优化器”更新为“SG Skill 优化师”。
- 仓库治理规则新增命名原则：面向用户的中文 Skill 名称在表达自然时优先采用拟人化、专业角色称谓，而不是机器或工具称谓。
- `VERSION`、下载链接和本地可复现构建产物同步升级为 `3.0.2`。
