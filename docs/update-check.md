...
当前检查器在 Skill 被实际调用时读取官方源码版本，仅提醒新版，不下载、安装或执行更新。主任务始终优先。它不是后台定时器，也不创建自动任务。
...
源码版本与 Release 独立：根 `VERSION` 表示当前源码版本，[最新稳定版](https://github.com/sgskills/aibp/releases/latest) 表示已经发布的安装包，两者在发布准备期可以不同。检查器仍以源码 VERSION 为真源；源码新版提醒不承诺同时存在同版本 tag、Release 或安装包。
...
实施期间的备份、进度与失败证据保存在 `.work/<VERSION>/`，不得提交或打包。回滚仅恢复本轮确认范围内的文件和构建产物，保留其他人的改动；恢复后重新运行验证，不重写已发布的历史。
