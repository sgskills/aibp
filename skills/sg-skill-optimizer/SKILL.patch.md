...
## [2026-09-08] 更新检查接入能力（规范 v1，未发布）

- 将“为其他 Skill 接入 30 天惰性检查”纳入诊断、实施计划和复验流程，附自包含规范、报告字段与 7 个模型场景。
- AIBP 必查入口、运行文件、固定真源、缓存/降级、动态纳入及独立打包；其他仓库需确认自己的真源，私有离线目标不默认联网。
- 只在确认范围内实施，只提醒、不自动更新；不修改通用静态评分、原有断言或 AIBP VERSION。
- 原有优化师单测 21/21、脚本 Golden Set 6/6 已实跑通过；新增模型场景与运行时回归分开记录，不把场景文件存在当作已验证。
- 本机副本保留既有正文差异；补齐原先缺失的自带测试资源，不改变算法。回滚使用本次备份恢复所改文件，移除本轮新建文件，不覆盖其他改动。



## [2026-09-06] AIBP 3.0.7 惰性检查更新

- 新增统一的 30 天惰性检查入口及独立运行文件；仅提醒官方源码新版，不下载或执行更新。
- 尊重 READ_ONLY、禁止联网、零写入及确认门；检查故障不阻断优化主任务。
- 原有 v2.5.0 方法、路由、评分与断言保持不变；AIBP 安装版本由独立文件管理。
